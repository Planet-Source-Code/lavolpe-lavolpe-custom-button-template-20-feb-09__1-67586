...
Option Explicit
' This instance of the template has drawing routines for the sample project
' The other instance in your zip file under the ResuableTemplate folder does not and is strictly a bare-bones template
...
' BUTTON TEMPLATE CODE\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    ...
    Dim wRect As RECT, dRect As RECT, lEdgeType As Long, lEdgeFlag As Long
    Dim sCaption As String
    Dim dtFlags As Long             ' flags used when rendering the caption
    Dim dtCalcFlags As Long         ' flags used when calculating caption rectangle
    Const gapOffsetX As Long = 4&   ' "non-client" start from left edge of control
    Const gapOffsetY As Long = 4&   ' "non-client" start from top edge of control
    
    ' get dimensions of our control in pixels. Note. If ScaleMode=vbPixels, then don't need the ScaleX/Y methods
    wRect.Right = ScaleX(UserControl.ScaleWidth, UserControl.ScaleMode, vbPixels)
    wRect.Bottom = ScaleY(UserControl.ScaleHeight, UserControl.ScaleMode, vbPixels)
    
    If Action = baDrawEntire Then
        
        Debug.Print ".. uc full repaint" ' testing purposes
        ...
        ' simple caption rendering. If more caption options are available, you may want to
        ' add this to a separate routine to support different colors, unicode, etc
    
        ' our caption calculation rectangle. Adjust for border & gap
        SetRect dRect, 0, 0, wRect.Right - gapOffsetX - gapOffsetX, wRect.Bottom - gapOffsetY - gapOffsetY
        
'        Example: if you want to use single line captions & truncate/ellipse the caption
'        sCaption = m_Caption    ' copy. We will allow API to truncate long captions in DrawText
'        dtFlags = DT_SINGLELINE
'        dtCalcFlags = DT_MODIFYSTRING Or DT_WORD_ELLIPSIS Or dtFlags
'        ' this next line will truncate and add ... to end of caption if it is too long
'        DrawText UserControl.hdc, sCaption, -1, dRect, DT_CALCRECT Or DT_NOCLIP Or dtCalcFlags
'        ' ^^ modifies rect to exact size, now we center, left, right align rect as needed
        
        ' Example: of multiline captions or change dtFlags below to DT_SINGLELINE only
        ' if non-truncated single line captions are desired. See above for truncating captions
        sCaption = m_Caption
        dtFlags = DT_WORDBREAK Or DT_MULTILINE
        dtCalcFlags = dtFlags
        DrawText UserControl.hdc, m_Caption, -1, dRect, DT_CALCRECT Or DT_NOCLIP Or dtCalcFlags
        ' ^^ modifies rect to exact size, now we center, left, right align rect as needed
        
'        ' Left Align Example
'        dtFlags = dtFlags Or DT_LEFT   ' and adjust the rect to our control (n/a for single line)
'        OffsetRect dRect, -dRect.Left + gapOffsetY, (wRect.Bottom - dRect.Bottom) \ 2
        
'        ' Right Align Example
'        dtFlags = dtFlags Or DT_RIGHT  ' and adjust the rect to our control (n/a for single line)
'        OffsetRect dRect, wRect.Right - dRect.Right - gapOffsetY, (wRect.Bottom - dRect.Bottom) \ 2
        
        ' Centering example
        dtFlags = dtFlags Or DT_CENTER  ' and adjust the rect to our control (n/a for single line)
        OffsetRect dRect, (wRect.Right - dRect.Right) \ 2, (wRect.Bottom - dRect.Bottom) \ 2
        
        ' Note. If a caption is several lines to long, VB centers the caption & clips so that
        ' you see the center out. Personal preferences. I don't like that. I think it is more
        ' profressional to show the top line of the caption in this case...
        If dRect.Top < gapOffsetY Then dRect.Top = gapOffsetY ' ensure we don't extend past top/bottom of button
        If dRect.Bottom > wRect.Bottom - gapOffsetY Then dRect.Bottom = wRect.Bottom - gapOffsetY
        
        ' draw the caption
        If GraphicalState = bdPushed Then OffsetRect dRect, 1, 1 ' make caption draw 1 pixel down,right
        DrawText UserControl.hdc, sCaption, -1, dRect, dtFlags
        
        ' replicating VB's button scheme styles
        If GraphicalState = bdPushed Then
            ' down state; 2-border levels only
            DrawBorder vb3DDKShadow, vb3DDKShadow, 0&   ' solid darker border on outer edge
            DrawBorder vb3DShadow, vb3DShadow, 1&       ' solid dark border on inside edge
        ElseIf (Attributes And attrIsDefaultBtn) Then
            ...
            ' up/hover states with focus/default button state; 3 border levels
            DrawBorder vb3DDKShadow, vb3DDKShadow, 0&   ' solid darker border on outer edge
            DrawBorder vb3DHighlight, vb3DDKShadow, 1&  ' 2-tone border on inner edge
            DrawBorder vb3DLight, vb3DShadow, 2&        ' 2-tone border on far inside edge
        Else ' up/hover/disabled without focus/default button state; 2 border levels
            DrawBorder vb3DHighlight, vb3DDKShadow, 0&  ' 2-tone outer border
            DrawBorder vb3DLight, vb3DShadow, 1&        ' 2-tone inside border
        End If
        
        UserControl.ForeColor = vbButtonText
        If (Attributes And attrHasFocus) Then           ' draw focus rect if needed
            SetRect wRect, gapOffsetX, gapOffsetY, wRect.Right - gapOffsetX, wRect.Bottom - gapOffsetY
            DrawFocusRect UserControl.hdc, wRect
        End If
            
    
    ElseIf Action = baDrawDefaultBdrOnly Then
    
        Debug.Print ".. uc redrawing default border only" ' testing purposes
        
        ' Draw button border
        ' Note about the Default property. If set you should draw a border as if the control had focus
        If (Attributes And attrIsDefaultBtn) Then
            ' add a solid border on the far outside
            DrawBorder vb3DDKShadow, vb3DDKShadow, 0&
            DrawBorder vb3DHighlight, vb3DDKShadow, 1&
            DrawBorder vb3DLight, vb3DShadow, 2&
        Else
            DrawBorder vb3DHighlight, vb3DDKShadow, 0&
            DrawBorder vb3DLight, vb3DShadow, 1&
            ' the previous border may have written over the 3rd level; revert that level to backcolor
            DrawBorder UserControl.BackColor, UserControl.BackColor, 2&
        End If

    ElseIf Action = baDrawFocusOnly Then
    
        Debug.Print ".. uc redrawing focusrect only" ' testing purposes
...
Private Sub DrawBorder(Color1 As Long, Color2 As Long, Offset As Long)
    ' simple border drawing routine, uses VB's Line method, one can use APIs if desired
    With UserControl
        If Color1 = Color2 Then
            ' can use the Rectangle API instead
            UserControl.Line (Offset, Offset)-(.ScaleWidth - Offset - 1, .ScaleHeight - Offset - 1), Color1, B
        Else
            ' can use the MoveToEx, LineTo APIs instead
            UserControl.Line (Offset, .ScaleHeight - Offset - 1)-(Offset, Offset), Color1
            UserControl.Line (Offset, Offset)-(.ScaleWidth - Offset - 1, Offset), Color1
            UserControl.Line (.ScaleWidth - Offset - 1, Offset)-(.ScaleWidth - Offset - 1, .ScaleHeight - Offset - 1), Color2
            UserControl.Line (Offset, .ScaleHeight - Offset - 1)-(.ScaleWidth - Offset, .ScaleHeight - Offset - 1), Color2
        End If
    End With
End Sub
